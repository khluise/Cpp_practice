...
bool checkWinner(char *spaces, char player, char computer){
    if ((spaces[0] != ' ' && spaces[0] == spaces[1] &&
         spaces[1] == spaces[2]) ||
        (spaces[3] != ' ' && spaces[3] == spaces[4] &&
         spaces[4] == spaces[5]) ||
        (spaces[6] != ' ' && spaces[6] == spaces[7] &&
         spaces[7] == spaces[8]) ||
        (spaces[0] != ' ' && spaces[0] == spaces[3] &&
         spaces[3] == spaces[6]) ||
        (spaces[1] != ' ' && spaces[1] == spaces[4] &&
         spaces[4] == spaces[7]) ||
        (spaces[2] != ' ' && spaces[2] == spaces[5] &&
         spaces[5] == spaces[8]) ||
        (spaces[0] != ' ' && spaces[0] == spaces[4] &&
         spaces[4] == spaces[8]) ||
        (spaces[2] != ' ' && spaces[2] == spaces[4] &&
         spaces[4] == spaces[6]))
    {
        char winner = ' ';
        if (spaces[0] != ' ' && spaces[0] == spaces[1] &&
